...
public class AvanceServiceImpl implements AvanceService {
	 ...
	 * @return
	 */
	private Position calculerDirection(Position position, Position newPosition, Avance avance) {
		if (Avance.G.equals(avance)) {
			String newDirection = position.getOrientation().getGauche();
			newPosition.setOrientation(Orientation.valueOf(newDirection));
		} else if (Avance.D.equals(avance)) {
			String newDirection = position.getOrientation().getDroite();
			newPosition.setOrientation(Orientation.valueOf(newDirection));
		} else if (Avance.A.equals(avance)) {
	 ...
	 * @return
	 */
	private Position avance(Position position, Position newPosition, Avance avance) {
